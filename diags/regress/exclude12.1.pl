...
# Perform generate
&runGenerateCommand( "-t main -v exclude12.1.v" );

# Simulate and get coverage information
if( $SIMULATOR eq "IV" ) {
  system( "iverilog -DDUMP covered/verilog/exclude12.1.v; ./a.out" ) && die;
} elsif( $SIMULATOR eq "CVER" ) {
  system( "cver -q +define+DUMP covered/verilog/exclude12.1.v" ) && die;
} elsif( $SIMULATOR eq "VCS" ) {
  system( "vcs +define+DUMP covered/verilog/exclude12.1.v; ./simv" ) && die;
} elsif( $SIMULATOR eq "VERIWELL" ) {
  system( "veriwell +define+DUMP covered/verilog/exclude12.1.v" ) && die;
}

# Perform diagnostic running code here
&runScoreCommand( "-vcd exclude12.1.vcd exclude12.1" );

# Create temporary file that will contain an exclusion message
&runCommand( "echo This toggle is not needed > exclude12.1.excl" );

# Perform exclusion
&runExcludeCommand( "-m T01 exclude12.1 < exclude12.1.excl" );
...
&runExcludeCommand( "-p T01 exclude12.1 > exclude12.1.out" );
if( $check == 1 ) {
