...
# Perform generate
&runGenerateCommand( "-t main -v exclude12.6.v" );

# Simulate and get coverage information
if( $SIMULATOR eq "IV" ) {
  system( "iverilog -DDUMP covered/verilog/exclude12.6.v; ./a.out" ) && die;
} elsif( $SIMULATOR eq "CVER" ) {
  system( "cver -q +define+DUMP covered/verilog/exclude12.6.v" ) && die;
} elsif( $SIMULATOR eq "VCS" ) {
  system( "vcs +define+DUMP covered/verilog/exclude12.6.v; ./simv" ) && die;
} elsif( $SIMULATOR eq "VERIWELL" ) {
  system( "veriwell +define+DUMP covered/verilog/exclude12.6.v" ) && die;
}

# Perform diagnostic running code here
&runScoreCommand( "-vcd exclude12.6.vcd exclude12.6" );

# Create temporary file that will contain an exclusion message
system( "touch exclude12.6.excl" ) && die;
  
# Perform exclusion
&runExcludeCommand( "-m L10 exclude12.6 < exclude12.6.excl" );
...
&runExcludeCommand( "-p L10 exclude12.6 > exclude12.6.out" );
if( $check == 1 ) {
