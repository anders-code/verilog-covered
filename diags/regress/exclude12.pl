...
if( $SIMULATOR eq "IV" ) {
  system( "iverilog -DDUMP covered/verilog/exclude12.v; ./a.out" ) && die;
} elsif( $SIMULATOR eq "CVER" ) {
  system( "cver -q +define+DUMP covered/verilog/exclude12.v" ) && die;
} elsif( $SIMULATOR eq "VCS" ) {
  system( "vcs +define+DUMP covered/verilog/exclude12.v; ./simv" ) && die;
} elsif( $SIMULATOR eq "VERIWELL" ) {
  system( "veriwell +define+DUMP covered/verilog/exclude12.v" ) && die;
}

# Perform diagnostic running code here
&runScoreCommand( "-vcd exclude12.vcd exclude12" );

# Create temporary file that will contain an exclusion message
&runCommand( "echo This line is not needed > exclude12.excl" );

# Perform exclusion
&runExcludeCommand( "-m L10 exclude12 < exclude12.excl" );
...
&runExcludeCommand( "-p L10 exclude12 > exclude12.out" );
if( $check == 1 ) {
