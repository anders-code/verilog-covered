...
# Perform generate
&runGenerateCommand( "-t main -v exclude13.v" );

# Simulate and get coverage information
if( $SIMULATOR eq "IV" ) {
  system( "iverilog -DDUMP covered/verilog/exclude13.v; ./a.out" ) && die;
} elsif( $SIMULATOR eq "CVER" ) {
  system( "cver -q +define+DUMP covered/verilog/exclude13.v" ) && die;
} elsif( $SIMULATOR eq "VCS" ) {
  system( "vcs +define+DUMP covered/verilog/exclude13.v; ./simv" ) && die;
} elsif( $SIMULATOR eq "VERIWELL" ) {
  system( "veriwell +define+DUMP covered/verilog/exclude13.v" ) && die;
}

# Perform diagnostic running code here
&runScoreCommand( "-vcd exclude13.vcd exclude13" );
...
# Perform exclusion
&runExcludeCommand( "-m L10 exclude13 < exclude13.excl" );
...
&runExcludeCommand( "-p L10 exclude13 > exclude13.out" );
if( $check == 1 ) {
