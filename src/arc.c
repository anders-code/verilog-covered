...
void arc_db_write(
  ...
  for( i=0; i<table->num_fr_states; i++ ) {
    vector_db_write( table->fr_states[i], file, TRUE, FALSE, scoring );
    fprintf( file, "  " );
  }
  for( i=0; i<table->num_to_states; i++ ) {
    vector_db_write( table->to_states[i], file, TRUE, FALSE, scoring );
    fprintf( file, "  " );
  }
...
void arc_db_read(
  /*@out@*/ fsm_table** table,   /*!< Pointer to state transition arc array */
            ...
            bool        use_cov  /*!< Set to TRUE when the coverage data should be used */
) { PROFILE(ARC_DB_READ);
  ...
  Try {
    ...
    /*@=formatcode@*/
      ...
      *line += chars_read;
      ...
      for( i=0; i<num_fr_states; i++ ) {
        vector_db_read( &((*table)->fr_states[i]), line, use_cov );
      }
      ...
      /* Read in vectors */
      for( i=0; i<num_to_states; i++ ) {
        vector_db_read( &((*table)->to_states[i]), line, use_cov );
      }
...
void arc_db_merge(
  ...
  bool       use_cov  /*!< Set to TRUE if coverage data should be used in the vectors */
  ...
  /* Read in the table */
  arc_db_read( &table, line, use_cov );
