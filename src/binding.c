  ...
  if( scope_local( name ) || !bind_locally || (exp->op == EXP_OP_PASSIGN) ) {
    ...
    if( retval ) {
      ...
      if( !clear_assigned ) {
        ...
        /* Set is_constant value in expression supplemental field */
        exp->suppl.part.is_constant = (found_sig->suppl.part.type == SSUPPL_TYPE_PARAM) ? 1 : 0;
...
/* 
 $Log$
 Revision 1.71.4.1.2.1  2006/04/25 04:55:08  phase1geo
