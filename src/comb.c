        ...
        if( (exp->op == EXP_OP_SIG) || (exp->op == EXP_OP_PARAM) ) {
          ...
          *size   = strlen( tmpname );
          ...
          switch( exp->op ) {
            ...
            default              :
              rv = snprintf( user_msg, USER_MSG_LENGTH, "Internal error:  Unknown expression type in combination_underline_tree (%d)", exp->op );
  ...
  if( exp != NULL ) {
    ...
    if( exp->ulid != -1 ) {
      ...
      if( (exp->ulid > 0) && ((unsigned int)exp->ulid >= *exclude_size) ) {
        ...
        *reasons      = (char**)realloc_safe( *reasons, (sizeof( char* ) * exp->ulid), (sizeof( char* ) * (exp->ulid + 1)) );
