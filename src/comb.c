...
/*!
 ...
 \param ul     Specifies if this expression tree needs to be underlined.
...
*/
void combination_multi_expr_calc( expression* exp, int* ulid, bool ul, int* hit, float* total ) {
  ...
  if( exp != NULL ) {
    ...
    if( (exp->left != NULL) && (exp->op != exp->left->op) ) {
      ...
      combination_multi_expr_calc( exp->left, ulid, ul, hit, total );
    }
    ...
    if( (exp->right != NULL) && (exp->op != exp->right->op) ) {
      ...
      combination_multi_expr_calc( exp->right, ulid, ul, hit, total );
    }
...
bool combination_instance_summary( FILE* ofile, funit_inst* root, char* parent ) {
  ...
  if( strcmp( parent, "*" ) == 0 ) {
    strcpy( tmpname, obfuscate_name( root->name, 'i' ) );
  } else {
    snprintf( tmpname, 4096, "%s.%s", parent, obfuscate_name( root->name, 'i' ) );
  }
...
bool combination_funit_summary( FILE* ofile, funit_link* head ) {
  ...
  while( head != NULL ) {
    ...
    fprintf( ofile, "  %-30.30s    %-30.30s   %4d/%4.0f/%4.0f      %3.0f%%\n", 
             obfuscate_name( head->funit->name, 'f' ),
             get_basename( obfuscate_name( head->funit->filename, 'v' ) ),
            ...
            case EXP_OP_FUNC_CALL :
              if( (tfunit = funit_find_by_id( exp->stmt->exp->id )) != NULL ) {
                ...
                snprintf( user_msg, USER_MSG_LENGTH, "Internal error:  Could not find statement %d in module %s",
                          exp->stmt->exp->id, obfuscate_name( funit->name, 'f' ) );
...
void combination_multi_var_exprs( char** line1, char** line2, char** line3, expression* exp ) {
  ...
  if( exp != NULL ) {
    ...
    if( (exp->left != NULL) && (exp->op != exp->left->op) ) {
      ...
      if( exp->left->ulid == -1 ) {
        FILE*      tfile = fopen( "failing_expr", "w" );
        statement* stmt  = expression_get_root_statement( exp );
        assert( stmt != NULL );
        assert( tfile != NULL );
        expression_db_write_recursive( stmt->exp, tfile );
        fclose( tfile );
        printf( "Expression %d contains ulid of -1\n", exp->id );
      }
...
void combination_instance_verbose( FILE* ofile, funit_inst* root, char* parent ) {
  ...
  if( ((root->stat->comb_hit < root->stat->comb_total) && !report_covered) ||
    ...
    fprintf( ofile, "%s, File: %s, Instance: %s\n", pname, obfuscate_name( root->funit->filename, 'v' ), tmpname );
    fprintf( ofile, "    -------------------------------------------------------------------------------------------------------------\n" );
...
void combination_funit_verbose( FILE* ofile, funit_link* head ) {
  ...
  while( head != NULL ) {
    ...
    if( ((head->funit->stat->comb_hit < head->funit->stat->comb_total) && !report_covered) ||
      ...
      fprintf( ofile, "%s, File: %s\n", pname, obfuscate_name( head->funit->filename, 'v' ) );
      fprintf( ofile, "    -------------------------------------------------------------------------------------------------------------\n" );
...
/*
 $Log$
 Revision 1.137.8.1.4.5  2006/11/07 17:17:35  phase1geo
