...
func_unit* db_add_instance(
  ...
  if( ((found_funit_link = funit_link_find( funit->name, funit->suppl.part.type, db_list[curr_db]->funit_head )) != NULL) && (generate_top_mode == 0) ) {
    ...
    /* If the functional unit already exists, use it instead of the intermediate functional unit */
...
bool db_do_timestep(
  ...
  if( debug_mode ) {
    if( final ) {
      ...
      unsigned int rv = snprintf( user_msg, USER_MSG_LENGTH, "Performing timestep #%" FMT64 "d", time );
      assert( rv < USER_MSG_LENGTH );
      print_output( user_msg, DEBUG, __FILE__, __LINE__ );
    }
  }
