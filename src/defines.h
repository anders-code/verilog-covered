...
union esuppl_u {
  ...
  struct {
    ...
    control owned          :1;  /*!< Bit 22.  Mask bit = 0.  Temporary bit value by the score command to indicate
                                     if this expression is currently owned by a module parameter or not. */
  } part;
};
...
union psuppl_u {
  ...
  struct {
    ...
    control owns_expr: 1;       /*!< Specifies if this parameter owns its expr structure and should
                                     therefore deallocate it when it is deallocated */
  } part;
};
...
/*
 ...
 change and added mbit_sel7 diagnostic to verify is correctness.
