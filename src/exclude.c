...
/*!
 ...
 \param root_seen  Pointer to value that stores off if we have already handled line coverage for this line
...
*/
void exclude_expr_assign_and_recalc( expression* expr, func_unit* funit, bool excluded, bool set_line, bool* root_seen ) {
...
bool exclude_set_line_exclude( char* funit_name, int funit_type, int line, int value ) {
  ...
  bool        root_seen = FALSE;  /* Specifies if we have seen at least one root expression */
  ...
  if( (funitl = funit_link_find( &funit, funit_head )) != NULL ) {
    ...
    do {
      ...
      if( expl != NULL ) {
        exclude_expr_assign_and_recalc( expl->exp, funitl->funit, (value == 1), TRUE, &root_seen );
        expl   = expl->next;
        retval = TRUE;
      }
...
bool exclude_set_comb_exclude( char* funit_name, int funit_type, int expr_id, int uline_id, int value ) {
  ...
  bool        root_seen = FALSE;  /* Specifies if we have seen a root expression */
  ...
  if( (funitl = funit_link_find( &funit, funit_head )) != NULL ) {
    ...
    if( (expl = exp_link_find( &exp, funitl->funit->exp_head )) != NULL ) {
      if( (subexp = expression_find_uline_id( expl->exp, uline_id )) != NULL ) {
        exclude_expr_assign_and_recalc( subexp, funitl->funit, (value == 1), FALSE, &root_seen );
        retval = TRUE;
      }
    }
...
bool exclude_set_assert_exclude( char* funit_name, int funit_type, char* inst_name, int expr_id, int value ) {
  ...
  if( (inst = exclude_find_instance_from_funit_info( funit_name, funit_type )) != NULL ) {
    ...
    if( (expl = exp_link_find( &exp, curr_child->funit->exp_head )) != NULL ) {
      exclude_expr_assign_and_recalc( expl->exp, curr_child->funit, (value == 1), FALSE, &root_seen );
      retval = TRUE;
    }
...
/*
 $Log$
 Revision 1.14.2.1  2007/03/09 17:37:58  phase1geo
