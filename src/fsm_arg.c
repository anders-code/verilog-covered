    ...
    if( sscanf( *str, "%[a-zA-Z0-9_]\[%d:%d]%n", str_val, &msb, &lsb, &chars_read ) == 3 ) {
      *str = *str + chars_read;
      if( (mparm = mod_parm_find( str_val, funit->param_head )) != NULL ) {
        ...
        expr = expression_create( right, left, EXP_OP_MBIT_SEL, FALSE, curr_expr_id, 0, 0, 0, FALSE ); 
        curr_expr_id++;
	
	bind_add( 0, str_val, expr, funit );
      ...
      *str = *str + chars_read;
      if( (mparm = mod_parm_find( str_val, funit->param_head )) != NULL ) {
        ...
        expr = expression_create( right, left, EXP_OP_MBIT_POS, FALSE, curr_expr_id, 0, 0, 0, FALSE );
        curr_expr_id++;
	...
	bind_add( 0, str_val, expr, funit );
      ...
      *str = *str + chars_read;
      if( (mparm = mod_parm_find( str_val, funit->param_head )) != NULL ) {
        ...
        /* Generate variable positive multi-bit parameter expression */
        expr = expression_create( right, left, EXP_OP_MBIT_NEG, FALSE, curr_expr_id, 0, 0, 0, FALSE );
        curr_expr_id++;
	...
	bind_add( 0, str_val, expr, funit );
      ...
      *str = *str + chars_read;
      if( (mparm = mod_parm_find( str_val, funit->param_head )) != NULL ) {
        ...
        expr = expression_create( NULL, left, EXP_OP_SBIT_SEL, FALSE, curr_expr_id, 0, 0, 0, FALSE );
        curr_expr_id++;
...
/*
 $Log$
 Revision 1.26.6.1  2006/04/24 22:54:24  phase1geo
