...
void fsm_arg_parse_trans( expression* expr, fsm* table, func_unit* funit ) {
  ...
  if( (from_state = fsm_arg_parse_value( &str, funit )) == NULL ) {
    snprintf( user_msg, USER_MSG_LENGTH, "Left-hand side FSM transition value must be a constant value or parameter, line: %d, file: %s",
              expr->line, obfuscate_name( funit->filename, 'v' ) );
    ...
    if( index == 1 ) {
      ...
      if( fsml == NULL ) {
        ...
        if( (in_state = fsm_arg_parse_state( &str, funit->name )) == NULL ) {
          snprintf( user_msg, USER_MSG_LENGTH, "Illegal input state expression (%s), file: %s",
                    str, obfuscate_name( funit->filename, 'v' ) );
          print_output( user_msg, FATAL, __FILE__, __LINE__ );
          exit( 1 );
        }
        free_safe( tmp );
        ...
        snprintf( user_msg, USER_MSG_LENGTH, "Input state specified after output state for this FSM has already been specified, file: %s",
                  obfuscate_name( funit->filename, 'v' ) );
        print_output( user_msg, FATAL, __FILE__, __LINE__ );
        exit( 1 );
      }
      ...
      if( fsml == NULL ) {
        ...
        if( (out_state = fsm_arg_parse_state( &str, funit->name )) == NULL ) {
          snprintf( user_msg, USER_MSG_LENGTH, "Illegal output state expression (%s), file: %s",
                    str, obfuscate_name( funit->filename, 'v' ) );
          print_output( user_msg, FATAL, __FILE__, __LINE__ );
          exit( 1 );
        } else {
      ...
      if( fsml == NULL ) {
        snprintf( user_msg, USER_MSG_LENGTH, "Attribute FSM name (%s) has not been previously created, file: %s",
                  obfuscate_name( table.name, 'n' ), obfuscate_name( funit->filename, 'v' ) );
        print_output( user_msg, FATAL, __FILE__, __LINE__ );
        exit( 1 );
      } else {
...
/*
 $Log$
 Revision 1.26.12.1  2006/08/18 04:50:44  phase1geo
