  ...
  if( (funitl = funit_link_find( &funit, funit_head )) != NULL ) {
    if( !bind_signal( sig_name, expr, funitl->funit, TRUE, FALSE, FALSE, expr->line, FALSE ) ) {
      snprintf( user_msg, USER_MSG_LENGTH, "Unable to bind FSM-specified signal (%s) to expression (%d) in module (%s)",
                obfuscate_name( sig_name, 's' ), expr->id, obfuscate_name( funit_name, 'f' ) );
      print_output( user_msg, FATAL, __FILE__, __LINE__ );
      retval = FALSE;
    }
  } else {
    snprintf( user_msg, USER_MSG_LENGTH, "Unable to find FSM-specified module (%s) in design",
              obfuscate_name( funit_name, 'f' ) ); 
    print_output( user_msg, FATAL, __FILE__, __LINE__ );
    retval = FALSE;
  }
...
/*
 $Log$
 Revision 1.24.12.1  2006/08/18 04:50:50  phase1geo
