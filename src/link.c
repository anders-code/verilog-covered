...
stmt_link* stmt_link_find_by_pos(
  unsigned int ppfline,     /*! Preprocessor line number of statement to find */
  ...
  curr = head;
  while( (curr != NULL) && ((curr->stmt->exp->ppfline != ppfline) || (curr->stmt->exp->col.part.first != first_col)) ) {
    curr = curr->next;
  }
...
exp_link* exp_link_find_by_pos(
  ...
  unsigned int fline,  /*!< First line of expression */
  unsigned int lline,  /*!< Last line of expression */
  ...
  curr = head;
  while( (curr != NULL) && ((curr->exp->ppfline != fline) || (curr->exp->pplline != lline) || (curr->exp->col.all != col) || (curr->exp->op != op)) ) {
    curr = curr->next;
  }
