...
static lxtint64_t vcd_prevtime     = 0;      /*! Specifies the last timestamp simulated */
static bool       use_vcd_prevtime = FALSE;  /*! Set to TRUE when vcd_prevtime has been set */
...
void vcd_callback(struct lxt2_rd_trace **lt, lxtint64_t *pnt_time, lxtint32_t *pnt_facidx, char **pnt_value) {
  ...
  if( (vcd_prevtime != *pnt_time) && use_vcd_prevtime ) {
    db_do_timestep( vcd_prevtime, FALSE );
  }
  vcd_prevtime     = *pnt_time;
  use_vcd_prevtime = TRUE;
