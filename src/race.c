...
void race_handle_race_condition( expression* expr, func_unit* mod, statement* stmt, statement* base, int reason ) {
  ...
  if( base == NULL ) {
    ...
    if( flag_race_check != NORMAL ) {
      ...
      snprintf( user_msg, USER_MSG_LENGTH, "  Signal assigned in file: %s, line: %d",
                obfuscate_name( mod->filename, 'v' ), expr->line );
      ...
      if( flag_race_check == WARNING ) {
        ...
        snprintf( user_msg, USER_MSG_LENGTH, "    Statement block starting at file: %s, line: %d",
                  obfuscate_name( mod->filename, 'v' ), stmt->exp->line );
  ...
  /* If the stmt and base pointers are pointing to different statements, we will output conflict for stmt */
    ...
    if( flag_race_check != NORMAL ) {
      ...
      snprintf( user_msg, USER_MSG_LENGTH, "  Signal also assigned in statement starting at file: %s, line: %d",
                obfuscate_name( mod->filename, 'v' ), base->exp->line );
      print_output( user_msg, (flag_race_check + 1), __FILE__, __LINE__ );
...
void race_report_verbose( FILE* ofile, funit_link* head ) {
  ...
  while( head != NULL ) {
    ...
    if( head->funit->stat->race_total > 0 ) {
      ...
      fprintf( ofile, "%s, File: %s\n", obfuscate_name( head->funit->name, 'f' ), obfuscate_name( head->funit->filename, 'v' ) );
      fprintf( ofile, "    -------------------------------------------------------------------------------------------------------------\n" );
