...
void vsignal_vcd_assign( vsignal* sig, char* value, int msb, int lsb ) {
  ...
  snprintf( user_msg, USER_MSG_LENGTH, "Assigning vsignal %s[%d:%d] (lsb=%d) to value %s",
            obfuscate_name( sig->name, 's' ), msb, lsb, sig->lsb, value );
...
void vsignal_display( vsignal* sig ) {
  ...
  printf( "  Signal =>  name: %s, lsb: %d, ", obfuscate_name( sig->name, 's' ), sig->lsb );
...
/*
 ...
 Revision 1.22.4.1.4.1.4.1  2006/08/11 04:13:14  phase1geo
