...
vsignal* vsignal_create( char* name, int width, int lsb ) {
  ...
  vsignal* new_sig;       /* Pointer to newly created signal */

  new_sig = (vsignal*)malloc_safe( sizeof( vsignal ) );

  vsignal_init( new_sig, strdup( name ), vector_create( width, lsb, TRUE ) );
...
bool vsignal_db_read( char** line, module* curr_mod ) {
  ...
  vsignal*    sig;            /* Pointer to the newly created signal              */
  ...
  if( sscanf( *line, "%s %s %n", name, modname, &chars_read ) == 2 ) {
    ...
    *line = *line + chars_read;
    ...
    if( vector_db_read( &vec, line ) ) {
      ...
      /* Create new signal */
      sig = vsignal_create( name, vec->width, vec->lsb );
